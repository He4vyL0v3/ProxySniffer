...
class ProxyCheckerApp(ctk.CTk):
    ...
    def setup_ui(self):
        self.header = ctk.CTkLabel(self, text="PROXY CHECKER", 
                                  font=("Arial", 30, "bold"))
        self.header.pack(pady=(20, 10))
        
        ctk.CTkLabel(self, text="Proxy Type:",
                    font=("Arial", 13), text_color="black").pack(anchor="w", padx=20, pady=5)
        
        self.proxy_type = ctk.CTkComboBox(self,
                                         values=["HTTP", "HTTPS", "SOCKS4", "SOCKS5"],
                                         state="readonly")
        self.proxy_type.set("HTTP")
        self.proxy_type.pack(fill="x", padx=20, pady=(0, 10))
        
        ctk.CTkLabel(self, text="Proxy Limit:", 
                    font=("Arial", 13), text_color="black").pack(anchor="w", padx=20, pady=5)
        
        self.limit_entry = ctk.CTkEntry(self, placeholder_text="Enter limit")
        self.limit_entry.pack(fill="x", padx=20, pady=(0, 10))
        
        ctk.CTkLabel(self, text="Download Progress:",
                    font=("Arial", 13), text_color="black").pack(anchor="w", padx=20, pady=(5, 0), )
        self.download_progress = ctk.CTkProgressBar(self)
        self.download_progress.pack(fill="x", padx=20, pady=5)
        self.download_progress.set(0)
        
        ctk.CTkLabel(self, text="Check Progress:",
                    font=("Arial", 13), text_color="black").pack(anchor="w", padx=20, pady=(5, 0))
        self.check_progress = ctk.CTkProgressBar(self)
        self.check_progress.pack(fill="x", padx=20, pady=5)
        self.check_progress.set(0)
        
        self.status_label = ctk.CTkLabel(self, text="Ready to start", 
                                        text_color="black")
        self.status_label.pack(pady=5)
        
        self.start_btn = ctk.CTkButton(self, text="Start Checking", 
                                      command=self.start_process, text_color="black")
        self.start_btn.pack(pady=10, padx=20, fill="x")
        
        self.save_btn = ctk.CTkButton(self, text="Save Results", 
                                     state="disabled", command=self.save_results, text_color="black")
    ...
    def start_process(self):
        ...
        threading.Thread(target=self.run_checking_process, 
                        args=(proxy_type, limit), 
                        daemon=True).start()
    ...
    def run_checking_process(self, proxy_type, limit):
        ...
        self.update_status(f"Found {len(self.working_proxies)} working proxies", "green")
        self.results.configure(state="normal")
        self.results.delete("1.0", "end")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_proxy = {
                executor.submit(
                    ...
                    proxy, 
                    proxy_type
                ): proxy for proxy in proxies_list
            }
    ...
    def check_proxy(self, proxy, proxy_type, test_url="https://www.example.com/", timeout=1):
    ...
    def update_status(self, message, color="white"):
        ...
        self.status_label.configure(text=message, 
                                   text_color=colors.get(color, "#ffffff"))
        self.update()
